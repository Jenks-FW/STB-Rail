...
tempCSX %<>%  mutate(grp = cumsum(str_detect(com_id, "^1$|^01$"))) %>% 
  group_by(grp) #%>% mutate(year = 

for (i in 1:nrow(tempCSX)) { 
  n <- tempCSX$grp[i] %% 5
  if(n == 0) {n <- 5}
  tempCSX$quarter[i] <- quarter[n]
}
...
colnames(tempCSX) <- c("commodity_id",
                        ...
                        "quarter",
                        "year")
tempCSX %<>% select(year, quarter, everything())
