...
tempNS %<>%  mutate(grp = cumsum(str_detect(com_id, "^1$|^01$"))) %>% 
  group_by(grp) #%>% mutate(year = 

for (i in 1:nrow(tempNS)) { 
  n <- tempNS$grp[i] %% 5
  if(n == 0) {n <- 5}
  tempNS$quarter[i] <- quarter[n]
}
...
colnames(tempNS) <- c("commodity_id",
...
tempNS %<>% select(year, quarter, everything())
